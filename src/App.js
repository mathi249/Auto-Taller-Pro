...
// ─── SUPABASE CONFIG ──────────────────────────────────────────
const SUPA_URL = "https://kudckvitrinuzbmxkrxd.supabase.co";
const SUPA_KEY = "sb_publishable_3VvENHBXOlMYiFzV58I1tg_RerLj5Zj";

const db = {
  async getOrdenes(tallerId) {
    const r = await fetch(`${SUPA_URL}/rest/v1/ordenes?taller_id=eq.${tallerId}&order=created_at.desc`, {
      headers: { apikey: SUPA_KEY, Authorization: `Bearer ${SUPA_KEY}` }
    });
    const data = await r.json();
    return data.map(o => ({
      id: o.id, cliente: o.cliente, auto: o.auto, patente: o.patente||"",
      fecha: o.fecha, estado: o.estado, presupuesto: o.presupuesto,
      precioFinal: o.precio_final, arreglos: o.arreglos||[],
    }));
  },
  async createOrden(tallerId, orden) {
    const r = await fetch(`${SUPA_URL}/rest/v1/ordenes`, {
      method: "POST",
      headers: { apikey: SUPA_KEY, Authorization: `Bearer ${SUPA_KEY}`, "Content-Type": "application/json", Prefer: "return=representation" },
      body: JSON.stringify({
        taller_id: tallerId, cliente: orden.cliente, auto: orden.auto,
        patente: orden.patente, fecha: orden.fecha, estado: orden.estado,
        presupuesto: orden.presupuesto||null, precio_final: orden.precioFinal||null,
        arreglos: orden.arreglos,
      })
    });
    const data = await r.json();
    const o = data[0];
    return { id: o.id, cliente: o.cliente, auto: o.auto, patente: o.patente||"", fecha: o.fecha, estado: o.estado, presupuesto: o.presupuesto, precioFinal: o.precio_final, arreglos: o.arreglos||[] };
  },
  async updateOrden(id, orden) {
    await fetch(`${SUPA_URL}/rest/v1/ordenes?id=eq.${id}`, {
      method: "PATCH",
      headers: { apikey: SUPA_KEY, Authorization: `Bearer ${SUPA_KEY}`, "Content-Type": "application/json" },
      body: JSON.stringify({
        cliente: orden.cliente, auto: orden.auto, patente: orden.patente,
        fecha: orden.fecha, estado: orden.estado,
        presupuesto: orden.presupuesto||null, precio_final: orden.precioFinal||null,
        arreglos: orden.arreglos,
      })
    });
...
// ─── HELPERS ─────────────────────────────────────────────────
const fmt = (n) => n != null && n !== "" ? "$" + Number(n).toLocaleString("es-AR", { minimumFractionDigits: 0 }) : "—";
...
</head><body><h1>⚙ AutoTaller Pro</h1><div class="tn">${tallerNombre}</div>
...
<div class="g"><div><div class="l">Cliente</div><div class="v">${orden.cliente}</div></div><div><div class="l">Vehículo</div><div class="v">${orden.auto}</div></div><div><div class="l">Patente</div><div class="v">${orden.patente||"—"}</div></div><div><div class="l">Fecha</div><div class="v">${fmtDate(orden.fecha)}</div></div><div><div class="l">Estado</div><div class="v">${orden.estado}</div></div><div><div class="l">Horas totales</div><div class="v">${th.toFixed(1)} hs</div></div></div>
...
<div class="tots"><div><div class="tl">Presupuesto</div><div class="tv">${fmt(orden.presupuesto)}</div></div><div><div class="tl">Precio Final</div><div class="tv fin">${fmt(orden.precioFinal)}</div></div></div>
<div class="foot">AutoTaller Pro — ${tallerNombre}</div></body></html>`;
  const w=window.open("","_blank","width=800,height=900"); w.document.write(html); w.document.close(); w.focus(); setTimeout(()=>w.print(),400);
};

const printAll = (registros, tallerNombre) => {
  const rows=registros.map(o=>{ const h=o.arreglos.reduce((s,a)=>s+(Number(a.horas)||0),0); return `<tr><td>${o.cliente}</td><td>${o.auto}</td><td>${o.patente||"—"}</td><td>${fmtDate(o.fecha)}</td><td>${o.estado}</td><td>${o.arreglos.length}</td><td style="text-align:right">${h.toFixed(1)} hs</td><td style="text-align:right">${fmt(o.presupuesto)}</td><td style="text-align:right;font-weight:700;color:#155724">${fmt(o.precioFinal)}</td></tr>`; }).join("");
  const total=registros.reduce((s,r)=>s+(Number(r.precioFinal)||0),0);
  const html=`<!DOCTYPE html><html lang="es"><head><meta charset="utf-8"/><title>Reporte General</title><style>body{font-family:Arial,sans-serif;margin:32px;color:#111;font-size:12px}h1{font-size:20px;margin-bottom:2px}.sub{color:#666;font-size:11px;margin-bottom:24px}table{width:100%;border-collapse:collapse}th{background:#111;color:#fff;padding:7px 8px;text-align:left;font-size:10px;letter-spacing:1px;text-transform:uppercase}td{padding:7px 8px;border-bottom:1px solid #eee}.fr td{font-weight:700;border-top:2px solid #111;background:#f9f9f9}.foot{margin-top:32px;padding-top:12px;border-top:1px solid #ddd;font-size:10px;color:#aaa;text-align:center}</style></head><body>
<h1>⚙ AutoTaller Pro — Reporte General</h1><div class="sub"><strong>${tallerNombre}</strong> · ${new Date().toLocaleDateString("es-AR",{day:"2-digit",month:"long",year:"numeric"})} · ${registros.length} órdenes</div>
...
<div class="foot">AutoTaller Pro — ${tallerNombre}</div></body></html>`;
  const w=window.open("","_blank","width=1000,height=800"); w.document.write(html); w.document.close(); w.focus(); setTimeout(()=>w.print(),400);
};

// ─── UI ATOMS ─────────────────────────────────────────────────
function Badge({estado}){
  const e=EC[estado]||EC["Pendiente"];
  return(<span style={{background:e.bg,border:`1px solid ${e.border}`,color:e.text,borderRadius:20,padding:"3px 10px",fontSize:11,fontWeight:600,display:"inline-flex",alignItems:"center",gap:5}}><span style={{width:5,height:5,borderRadius:"50%",background:e.dot,display:"inline-block"}}/>{estado}</span>);
}
...
function FInput({label,required,...p}){return(<div><label style={LBL}>{label}{required&&<span style={{color:"#e05c5c"}}> *</span>}</label><input {...p} style={INP}/></div>);}
function FSelect({label,value,onChange,options}){return(<div><label style={LBL}>{label}</label><select value={value} onChange={onChange} style={{...INP,cursor:"pointer"}}>{options.map(o=><option key={o}>{o}</option>)}</select></div>);}
function ArreglosEditor({arreglos,onChange}){
  ...
  return(<div style={{gridColumn:"1/-1"}}>
    ...
    {arreglos.map((a,i)=>(<div key={a.id} style={{background:"#111",border:"1px solid #222",borderRadius:9,padding:"14px",marginBottom:10,position:"relative"}}>
...
function LoginScreen({onLogin}){
  ...
  const handleLogin=useCallback(()=>{
    ...
    setTimeout(()=>{
      ...
      if(t){ onLogin(t); } else{ setError("Nombre de taller o clave incorrectos."); setLoading(false); }
    },400);
  ...
  return(<div style={{minHeight:"100vh",background:"#0a0a0a",display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
    <div style={{width:"100%",maxWidth:400}}>
      ...
      <div style={{background:"#141414",border:"1px solid #2a2a2a",borderRadius:16,padding:"32px 28px",boxShadow:"0 24px 64px rgba(0,0,0,0.6)"}}>
        ...
        <div style={{marginBottom:18}}><label style={LBL}>Nombre del taller</label><input value={nombre} onChange={e=>setNombre(e.target.value)} placeholder="Ej: Taller El Motor" onKeyDown={e=>e.key==="Enter"&&canSubmit&&handleLogin()} style={inpSt}/></div>
        <div style={{marginBottom:24}}><label style={LBL}>Clave</label><input type="password" value={clave} onChange={e=>setClave(e.target.value)} placeholder="••••••••" onKeyDown={e=>e.key==="Enter"&&canSubmit&&handleLogin()} style={inpSt}/></div>
...
export default function App(){
  const [tallerActivo,setTallerActivo]=useState(()=>{
    const sid=LS.getSession();
    if(!sid) return null;
    return TALLERES.find(t=>t.id===sid)||null;
  });
  ...
  // Cargar datos desde Supabase cuando cambia el taller
  useEffect(()=>{
    if(!tallerActivo) return;
    setLoading(true);
    db.getOrdenes(tallerActivo.id)
  ...
  const stats=useMemo(()=>{
    ...
    return{total:registros.length,ingresos,horas,enProceso:registros.filter(r=>r.estado==="En proceso").length,entregados:registros.filter(r=>r.estado==="Entregado").length};
  },[registros]);

  const showToast=useCallback((msg,type="ok")=>{ setToast({msg,type}); setTimeout(()=>setToast(null),3000); },[]);

  const handleLogin=useCallback((taller)=>{ LS.setSession(taller.id); setTallerActivo(taller); },[]);
  const handleLogout=useCallback(()=>{ LS.clrSession(); setTallerActivo(null); setRegistros([]); },[]);
  ...
  const saveOrden=async()=>{
    ...
    try{
      if(editId){
        await db.updateOrden(editId,p);
        setRegistros(prev=>prev.map(r=>r.id===editId?{...p,id:editId}:r));
        showToast("Orden actualizada ✓");
      } else {
        const nueva=await db.createOrden(tallerActivo.id,p);
        setRegistros(prev=>[nueva,...prev]);
        showToast("Orden creada ✓");
      }
      setModal(false);
    } catch(e){ showToast("Error al guardar. Verificá la conexión.","err"); }
    setSaving(false);
  };

  const doDelete=async()=>{
    try{
      await db.deleteOrden(deleteConfirm);
      setRegistros(p=>p.filter(r=>r.id!==deleteConfirm));
      showToast("Orden eliminada","err");
    } catch(e){ showToast("Error al eliminar","err"); }
    setDeleteConfirm(null); setDetalle(null);
  };

  const ordenDetalle=registros.find(r=>r.id===detalle);
  const fi=(f)=>({value:form[f],onChange:e=>setForm({...form,[f]:e.target.value})});

  return(<div style={{fontFamily:"'DM Sans',sans-serif",background:"#0f0f0f",minHeight:"100vh",color:"#f0ece0"}}>
    {toast&&<div style={{position:"fixed",bottom:24,right:24,zIndex:9999,background:toast.type==="err"?"#2a1010":"#0d2a1a",border:`1px solid ${toast.type==="err"?"#c0392b":"#1a6b3a"}`,borderRadius:10,padding:"12px 20px",fontSize:14,color:"#f0ece0",boxShadow:"0 8px 32px rgba(0,0,0,0.5)"}}>{toast.type==="err"?"🗑 ":"✅ "}{toast.msg}</div>}
    ...
    <header style={{background:"#0f0f0f",borderBottom:"1px solid #c8a84b33",padding:"0 28px",display:"flex",alignItems:"center",justifyContent:"space-between",height:68,position:"sticky",top:0,zIndex:200}}>
      <div style={{display:"flex",alignItems:"center",gap:12}}>
        ...
        <div><div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:22,letterSpacing:2,lineHeight:1}}>AutoTaller Pro</div><div style={{fontSize:11,color:"#c8a84b"}}>📍 {tallerActivo.nombre}</div></div>
      </div>
    ...
    <main style={{maxWidth:1180,margin:"0 auto",padding:"28px 20px"}}>
      <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:14,marginBottom:28}}>
        {[{label:"Órdenes",value:stats.total,icon:"📋",color:"#c8a84b"},{label:"En Proceso",value:stats.enProceso,icon:"🔧",color:"#74b9ff"},{label:"Entregados",value:stats.entregados,icon:"✅",color:"#6bcb8b"},{label:"Horas totales",value:stats.horas.toFixed(1)+" hs",icon:"⏱",color:"#fd79a8"},{label:"Ingresos",value:fmt(stats.ingresos),icon:"💰",color:"#f9ca24"}].map(s=>(
      ...
      <div style={{display:"flex",gap:10,marginBottom:20,flexWrap:"wrap",alignItems:"center"}}>
        <input placeholder="🔍  Buscar por cliente, vehículo o patente..." value={busqueda} onChange={e=>setBusqueda(e.target.value)} style={{flex:1,minWidth:240,background:"#141414",border:"1px solid #2a2a2a",borderRadius:8,padding:"9px 14px",color:"#f0ece0",fontSize:14,outline:"none"}}/>
        <select value={filtroEstado} onChange={e=>setFiltroEstado(e.target.value)} style={{background:"#141414",border:"1px solid #2a2a2a",borderRadius:8,padding:"9px 14px",color:"#f0ece0",fontSize:14,cursor:"pointer",outline:"none"}}>{["Todos",...ESTADOS].map(e=><option key={e}>{e}</option>)}</select>
        <div style={{display:"flex",gap:3,background:"#141414",borderRadius:8,padding:3,border:"1px solid #2a2a2a"}}>
          {[{v:"lista",icon:"☰"},{v:"cards",icon:"⊞"}].map(({v,icon})=>(<button key={v} onClick={()=>setVista(v)} style={{background:vista===v?"#c8a84b":"transparent",border:"none",borderRadius:5,padding:"6px 11px",color:vista===v?"#0f0f0f":"#888",cursor:"pointer",fontSize:15}}>{icon}</button>))}
        </div>
      </div>

      {loading&&<div style={{textAlign:"center",padding:"70px 0"}}><div style={{fontSize:40,marginBottom:14,animation:"spin 1s linear infinite",display:"inline-block"}}>⚙</div><div style={{fontSize:16,color:"#666"}}>Cargando órdenes...</div></div>}

      {!loading&&filtered.length===0&&<div style={{textAlign:"center",padding:"70px 0"}}><div style={{fontSize:52,marginBottom:14}}>🔧</div><div style={{fontSize:17,color:"#555"}}>{registros.length===0?"No hay órdenes todavía":"No hay resultados"}</div><div style={{fontSize:13,color:"#444",marginTop:6}}>Creá la primera con "+ Nueva Orden"</div></div>}

      {!loading&&filtered.length>0&&vista==="lista"&&(
        <div style={{background:"#141414",borderRadius:12,border:"1px solid #1e1e1e",overflow:"hidden"}}>
          <table style={{width:"100%",borderCollapse:"collapse"}}>
            <thead><tr style={{background:"#111",borderBottom:"1px solid #c8a84b22"}}>
              {["Cliente","Vehículo / Patente","Fecha","Arreglos","Horas","Presupuesto","Precio Final","Estado",""].map(h=>(<th key={h} style={{padding:"12px 14px",textAlign:"left",fontSize:10,letterSpacing:1.5,color:"#c8a84b",textTransform:"uppercase",fontWeight:600}}>{h}</th>))}
            </tr></thead>
            <tbody>
              {filtered.map((r,i)=>{
                const horas=r.arreglos.reduce((s,a)=>s+(Number(a.horas)||0),0);
                return(<tr key={r.id} style={{borderBottom:"1px solid #1e1e1e",background:i%2===0?"#141414":"#121212",cursor:"pointer"}} onMouseEnter={e=>e.currentTarget.style.background="#1e1e1e"} onMouseLeave={e=>e.currentTarget.style.background=i%2===0?"#141414":"#121212"}>
                  ...
                  <td style={{padding:"13px 10px"}}><div style={{display:"flex",gap:5}}>
      ...
      {!loading&&filtered.length>0&&vista==="cards"&&(
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(300px,1fr))",gap:18}}>
          {filtered.map(r=>{
            const horas=r.arreglos.reduce((s,a)=>s+(Number(a.horas)||0),0);
            return(<div key={r.id} style={{background:"#141414",border:"1px solid #1e1e1e",borderRadius:14,overflow:"hidden",cursor:"pointer",transition:"transform 0.2s,box-shadow 0.2s"}} onMouseEnter={e=>{e.currentTarget.style.transform="translateY(-3px)";e.currentTarget.style.boxShadow="0 10px 40px rgba(0,0,0,0.5)";}} onMouseLeave={e=>{e.currentTarget.style.transform="";e.currentTarget.style.boxShadow="";}} onClick={()=>setDetalle(r.id)}>
    ...
    {ordenDetalle&&(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",display:"flex",alignItems:"flex-start",justifyContent:"center",zIndex:500,padding:"24px 16px",overflowY:"auto"}} onClick={e=>e.target===e.currentTarget&&setDetalle(null)}>
      <div style={{background:"#141414",borderRadius:16,width:"100%",maxWidth:620,border:"1px solid #2a2a2a",boxShadow:"0 24px 80px rgba(0,0,0,0.7)"}}>
        ...
        <div style={{padding:"20px 24px"}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"10px 20px",marginBottom:22}}>
            {[["Fecha",fmtDate(ordenDetalle.fecha)],["Presupuesto",fmt(ordenDetalle.presupuesto)],["Precio Final",fmt(ordenDetalle.precioFinal)]].map(([l,v])=>(<div key={l}><div style={{fontSize:9,color:"#555",letterSpacing:1.5,textTransform:"uppercase",marginBottom:3}}>{l}</div><div style={{fontWeight:600,fontSize:15}}>{v}</div></div>))}
          </div>
          ...
          {ordenDetalle.arreglos.map((a,i)=>(<div key={a.id} style={{background:"#111",border:"1px solid #1e1e1e",borderRadius:9,padding:"12px 16px",marginBottom:10}}>
            <div style={{display:"flex",justifyContent:"space-between",gap:10}}>
              <div style={{flex:1}}><div style={{fontSize:10,color:"#555",marginBottom:4}}>ARREGLO #{i+1}</div><div style={{fontWeight:600,fontSize:14,marginBottom:a.materiales?6:0}}>{a.descripcion||"—"}</div>{a.materiales&&<div style={{fontSize:12,color:"#888"}}>🔩 {a.materiales}</div>}</div>
              {a.horas&&<div style={{background:"#2a1a2a",border:"1px solid #4a2a4a",borderRadius:8,padding:"6px 12px",textAlign:"center",flexShrink:0}}><div style={{fontSize:10,color:"#888"}}>HORAS</div><div style={{fontWeight:700,color:"#fd79a8",fontSize:16}}>{Number(a.horas).toFixed(1)}</div></div>}
            </div>
          </div>))}
    ...
    {modal&&(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.88)",display:"flex",alignItems:"flex-start",justifyContent:"center",zIndex:800,padding:"24px 16px",overflowY:"auto"}} onClick={e=>e.target===e.currentTarget&&setModal(false)}>
      <div style={{background:"#141414",borderRadius:16,width:"100%",maxWidth:600,border:"1px solid #c8a84b33",boxShadow:"0 24px 80px rgba(0,0,0,0.8)"}}>
        ...
        <div style={{padding:"0 26px 26px",display:"flex",gap:10,justifyContent:"flex-end"}}>
          ...
          <button onClick={saveOrden} disabled={!form.cliente||!form.auto||!form.fecha||saving}
            style={{background:(!form.cliente||!form.auto||!form.fecha||saving)?"#222":"linear-gradient(135deg,#c8a84b,#e8c96b)",border:"none",borderRadius:8,padding:"9px 26px",color:(!form.cliente||!form.auto||!form.fecha||saving)?"#555":"#0f0f0f",fontWeight:700,fontSize:14,cursor:"pointer"}}>
            {saving?"Guardando...":editId?"Guardar Cambios":"Crear Orden"}
          </button>
        </div>
      </div>
    </div>)}

    {deleteConfirm&&(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.9)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:900,padding:20}}>
