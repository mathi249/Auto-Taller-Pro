...
// Para agregar un taller nuevo, copiá una línea y cambiá los datos
const TALLERES = [
...
// ─── localStorage por taller ─────────────────────────────────
const loadData = (tallerId) => {
  try { const d = localStorage.getItem(`taller_data_${tallerId}`); return d ? JSON.parse(d) : SEED; }
  catch { return SEED; }
};
const saveData = (tallerId, data) => {
  try { localStorage.setItem(`taller_data_${tallerId}`, JSON.stringify(data)); } catch {}
};
const loadSession = () => { try { return localStorage.getItem("taller_session") || null; } catch { return null; } };
const saveSession = (id) => { try { localStorage.setItem("taller_session", id); } catch {} };
const clearSession = () => { try { localStorage.removeItem("taller_session"); } catch {} };
...
<style>body{font-family:Arial,sans-serif;margin:40px;color:#111;font-size:13px}h1{font-size:22px;margin-bottom:2px}.taller{font-size:14px;font-weight:700;color:#444;margin-bottom:24px}.grid{display:grid;grid-template-columns:1fr 1fr;gap:8px 24px;margin-bottom:24px}.label{font-size:10px;text-transform:uppercase;letter-spacing:1px;color:#888;margin-bottom:2px}.val{font-size:14px;font-weight:600}table{width:100%;border-collapse:collapse;margin-bottom:20px}th{background:#111;color:#fff;padding:8px 10px;text-align:left;font-size:11px;letter-spacing:1px;text-transform:uppercase}td{padding:8px 10px;border-bottom:1px solid #eee;vertical-align:top}.totals{display:flex;justify-content:flex-end;gap:40px;margin-top:8px}.tot-label{font-size:11px;color:#888;text-transform:uppercase}.tot-val{font-size:18px;font-weight:700;margin-top:2px}.final{color:#155724}.footer{margin-top:40px;padding-top:16px;border-top:1px solid #ddd;font-size:11px;color:#aaa;text-align:center}</style>
</head><body>
<h1>⚙ AutoTaller Pro</h1><div class="taller">${tallerNombre}</div>
<p style="color:#666;font-size:12px;margin-bottom:24px">Orden de Trabajo — ${new Date().toLocaleDateString("es-AR",{day:"2-digit",month:"long",year:"numeric"})}</p>
<div class="grid">
  ...
  <div><div class="label">Fecha</div><div class="val">${fmtDate(orden.fecha)}</div></div>
...
<div class="footer">AutoTaller Pro — ${tallerNombre}</div>
</body></html>`;
  const w = window.open("","_blank","width=800,height=900");
  w.document.write(html); w.document.close(); w.focus(); setTimeout(()=>w.print(),400);
};

const printAllReport = (registros, tallerNombre) => {
...
function Badge({ estado }) {
  ...
  return (
    <span style={{ background: e.bg, border: `1px solid ${e.border}`, color: e.text, borderRadius: 20, padding: "3px 10px", fontSize: 11, fontWeight: 600, display: "inline-flex", alignItems: "center", gap: 5 }}>
...
function FInput({ label, value, onChange, type = "text", placeholder, required }) {
  return (
    <div>
      <label style={{ display: "block", fontSize: 10, color: "#c8a84b", letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 5, fontFamily: "'Bebas Neue', sans-serif" }}>
        {label}{required && <span style={{ color: "#e05c5c" }}> *</span>}
      </label>
      <input type={type} value={value} onChange={onChange} placeholder={placeholder}
        style={{ width: "100%", background: "#141414", border: "1px solid #2a2a2a", borderRadius: 7, padding: "9px 13px", color: "#f0ece0", fontSize: 14, outline: "none", boxSizing: "border-box", transition: "border-color 0.2s" }}
...
function FSelect({ label, value, onChange, options }) {
  return (
    <div>
      <label style={{ display: "block", fontSize: 10, color: "#c8a84b", letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 5, fontFamily: "'Bebas Neue', sans-serif" }}>{label}</label>
      <select value={value} onChange={onChange}
        style={{ width: "100%", background: "#141414", border: "1px solid #2a2a2a", borderRadius: 7, padding: "9px 13px", color: "#f0ece0", fontSize: 14, outline: "none", boxSizing: "border-box", cursor: "pointer" }}>
...
function ArreglosEditor({ arreglos, onChange }) {
  ...
  const inputSt = { width: "100%", background: "#141414", border: "1px solid #2a2a2a", borderRadius: 6, padding: "8px 12px", color: "#f0ece0", fontSize: 13, outline: "none", boxSizing: "border-box" };
  const lblSt = { display: "block", fontSize: 10, color: "#888", letterSpacing: 1, textTransform: "uppercase", marginBottom: 4 };
  return (
    <div style={{ gridColumn: "1/-1" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
        <label style={{ fontSize: 10, color: "#c8a84b", letterSpacing: 1.5, textTransform: "uppercase", fontFamily: "'Bebas Neue', sans-serif" }}>Arreglos realizados</label>
      ...
      {arreglos.length === 0 && (
        <div style={{ background: "#111", border: "1px dashed #2a2a2a", borderRadius: 8, padding: "16px", textAlign: "center", color: "#555", fontSize: 13 }}>Sin arreglos. Hacé clic en "+ Agregar arreglo".</div>
      )}
      {arreglos.map((a, i) => (
        <div key={a.id} style={{ background: "#111", border: "1px solid #222", borderRadius: 9, padding: "14px", marginBottom: 10, position: "relative" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            <div style={{ gridColumn: "1/-1" }}>
              ...
              <input value={a.descripcion} onChange={e => upd(a.id, "descripcion", e.target.value)} placeholder="Ej: Cambio de correa de distribución" style={inputSt} />
            </div>
            <div>
              <label style={lblSt}>Horas de trabajo</label>
              <input type="number" min="0" step="0.5" value={a.horas} onChange={e => upd(a.id, "horas", e.target.value)} placeholder="Ej: 2.5" style={inputSt} />
            </div>
            <div>
              <label style={lblSt}>Materiales usados</label>
              <input value={a.materiales} onChange={e => upd(a.id, "materiales", e.target.value)} placeholder="Ej: Correa Gates, tensor" style={inputSt} />
            </div>
          </div>
      ...
      {arreglos.length > 0 && (
        <div style={{ textAlign: "right", fontSize: 12, color: "#888", marginTop: 4 }}>
          Total horas: <strong style={{ color: "#c8a84b" }}>{arreglos.reduce((s, a) => s + (Number(a.horas) || 0), 0).toFixed(1)} hs</strong>
        </div>
      )}
    </div>
  );
...
function LoginScreen({ onLogin }) {
  ...
  const handleLogin = () => {
    ...
    setTimeout(() => {
      const taller = TALLERES.find(
        t => t.nombre.toLowerCase() === nombre.trim().toLowerCase() && t.clave === clave.trim()
      );
      if (taller) { saveSession(taller.id); onLogin(taller); }
      else { setError("Nombre de taller o clave incorrectos."); }
      setLoading(false);
    }, 600);
  };
  ...
  const inputSt = {
    ...
    borderRadius: 8, padding: "11px 14px", color: "#f0ece0",
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0a0a0a", display: "flex", alignItems: "center", justifyContent: "center", padding: 20, position: "relative", overflow: "hidden" }}>
      <div style={{ position: "absolute", inset: 0, pointerEvents: "none" }}>
        <div style={{ position: "absolute", top: -120, left: -120, width: 400, height: 400, borderRadius: "50%", background: "radial-gradient(circle, #c8a84b15, transparent 70%)" }} />
        <div style={{ position: "absolute", bottom: -100, right: -100, width: 350, height: 350, borderRadius: "50%", background: "radial-gradient(circle, #c8a84b10, transparent 70%)" }} />
      </div>
      <div style={{ width: "100%", maxWidth: 400, position: "relative" }}>
        ...
        <div style={{ background: "#141414", border: "1px solid #2a2a2a", borderRadius: 16, padding: "32px 28px", boxShadow: "0 24px 64px rgba(0,0,0,0.6)" }}>
          ...
          <div style={{ marginBottom: 18 }}>
            <label style={{ display: "block", fontSize: 10, color: "#c8a84b", letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 6, fontFamily: "'Bebas Neue', sans-serif" }}>Nombre del taller</label>
            <input value={nombre} onChange={e => setNombre(e.target.value)} placeholder="Ej: Taller El Motor"
              onKeyDown={e => e.key === "Enter" && handleLogin()} style={inputSt}
              onFocus={e => e.target.style.borderColor = "#c8a84b"} onBlur={e => e.target.style.borderColor = "#2a2a2a"} />
          </div>

          <div style={{ marginBottom: 24 }}>
            <label style={{ display: "block", fontSize: 10, color: "#c8a84b", letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 6, fontFamily: "'Bebas Neue', sans-serif" }}>Clave</label>
          ...
          {error && (
            <div style={{ background: "#2a1010", border: "1px solid #5a2020", borderRadius: 8, padding: "10px 14px", color: "#e05c5c", fontSize: 13, marginBottom: 18 }}>⚠ {error}</div>
          )}

          <button onClick={handleLogin} disabled={!nombre || !clave || loading}
            style={{ width: "100%", padding: "12px", background: (!nombre || !clave || loading) ? "#222" : "linear-gradient(135deg, #c8a84b, #e8c96b)", border: "none", borderRadius: 8, color: (!nombre || !clave || loading) ? "#555" : "#0f0f0f", fontWeight: 700, fontSize: 15, cursor: "pointer", letterSpacing: 0.5 }}>
...
export default function App() {
  const [tallerActivo, setTallerActivo] = useState(() => {
    ...
    return sid ? TALLERES.find(t => t.id === sid) || null : null;
  });
  ...
  const filtered = registros.filter(r => {
    ...
    return (!q || r.cliente.toLowerCase().includes(q) || r.auto.toLowerCase().includes(q) || r.patente.toLowerCase().includes(q))
  ...
  const saveOrden = () => {
    ...
    const parsed = { ...form, presupuesto: form.presupuesto !== "" ? Number(form.presupuesto) : null, precioFinal: form.precioFinal !== "" ? Number(form.precioFinal) : null };
    if (editId) { setRegistros(p => p.map(r => r.id === editId ? { ...parsed, id: editId } : r)); showToast("Orden actualizada"); }
    else { setRegistros(p => [...p, { ...parsed, id: Date.now() }]); showToast("Orden creada"); }
    setModal(false);
  };
  ...
  const btnSt = (color) => ({ background: color === "gold" ? "linear-gradient(135deg,#c8a84b,#e8c96b)" : color === "blue" ? "#1a1a2a" : color === "dark" ? "#2a2a1a" : "#2a1a1a", border: `1px solid ${color === "gold" ? "none" : color === "blue" ? "#2a2a3a" : color === "dark" ? "#3a3a2a" : "#4a2a2a"}`, borderRadius: 6, padding: "5px 8px", color: color === "gold" ? "#0f0f0f" : color === "blue" ? "#74b9ff" : color === "dark" ? "#c8a84b" : "#e05c5c", cursor: "pointer", fontSize: 13 });

  return (
    <div style={{ fontFamily: "'DM Sans', sans-serif", background: "#0f0f0f", minHeight: "100vh", color: "#f0ece0" }}>

      {toast && (
        <div style={{ position: "fixed", bottom: 24, right: 24, zIndex: 9999, background: toast.type === "err" ? "#2a1010" : "#0d2a1a", border: `1px solid ${toast.type === "err" ? "#c0392b" : "#1a6b3a"}`, borderRadius: 10, padding: "12px 20px", fontSize: 14, color: "#f0ece0", boxShadow: "0 8px 32px rgba(0,0,0,0.5)", animation: "slideIn 0.3s ease" }}>
      ...
      <header style={{ background: "#0f0f0f", borderBottom: "1px solid #c8a84b33", padding: "0 28px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 68, position: "sticky", top: 0, zIndex: 200 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 11, color: "#c8a84b", letterSpacing: 1 }}>📍 {tallerActivo.nombre}</div>
          </div>
        </div>
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          <button onClick={() => printAllReport(filtered, tallerActivo.nombre)} style={{ background: "transparent", border: "1px solid #333", borderRadius: 8, padding: "8px 14px", color: "#aaa", cursor: "pointer", fontSize: 13 }}>📄 Reporte General</button>
          ...
          <button onClick={() => { clearSession(); setTallerActivo(null); setRegistros([]); }} title="Cerrar sesión" style={{ background: "#1a1a1a", border: "1px solid #2a2a2a", borderRadius: 8, padding: "9px 12px", color: "#666", cursor: "pointer", fontSize: 13 }}>🚪 Salir</button>
        </div>
      </header>
                  ...
                  return (
                    <tr key={r.id} style={{ borderBottom: "1px solid #1e1e1e", background: i % 2 === 0 ? "#141414" : "#121212", cursor: "pointer" }}
                      ...
                      <td style={{ padding: "13px 14px" }} onClick={() => setDetalle(r.id)}>
                        ...
                        <div style={{ fontSize: 11, color: "#c8a84b" }}>{r.patente}</div>
                      </td>
                      <td style={{ padding: "13px 14px", color: "#888", fontSize: 12 }} onClick={() => setDetalle(r.id)}>{fmtDate(r.fecha)}</td>
                      <td style={{ padding: "13px 14px" }} onClick={() => setDetalle(r.id)}>
                        <span style={{ background: "#1a2a1a", border: "1px solid #2a4a2a", borderRadius: 12, padding: "2px 10px", fontSize: 12, color: "#6bcb8b" }}>{r.arreglos.length} {r.arreglos.length === 1 ? "arreglo" : "arreglos"}</span>
                      </td>
                      ...
                      <td style={{ padding: "13px 10px" }}>
                        <div style={{ display: "flex", gap: 5 }}>
                          <button onClick={() => printReport(r, tallerActivo.nombre)} style={btnSt("blue")}>🖨</button>
                          <button onClick={() => openEdit(r)} style={btnSt("dark")}>✏️</button>
                          <button onClick={() => setDeleteConfirm(r.id)} style={btnSt("red")}>🗑</button>
                        </div>
                      </td>
                    </tr>
                  );
                  ...
                  <div style={{ padding: "14px 18px" }}>
                    ...
                    <div style={{ display: "flex", gap: 10 }}>
                      <div style={{ flex: 1, background: "#111", borderRadius: 8, padding: "9px 12px" }}>
                        ...
                        <div style={{ fontWeight: 700, color: "#888" }}>{fmt(r.presupuesto)}</div>
                      </div>
                      <div style={{ flex: 1, background: "#111", borderRadius: 8, padding: "9px 12px" }}>
                        <div style={{ fontSize: 9, color: "#555", letterSpacing: 1, marginBottom: 2 }}>PRECIO FINAL</div>
                        <div style={{ fontWeight: 700, color: r.precioFinal ? "#6bcb8b" : "#444" }}>{fmt(r.precioFinal)}</div>
                      </div>
                    </div>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", display: "flex", alignItems: "flex-start", justifyContent: "center", zIndex: 500, padding: "24px 16px", overflowY: "auto" }}
          ...
          <div style={{ background: "#141414", borderRadius: 16, width: "100%", maxWidth: 620, border: "1px solid #2a2a2a", boxShadow: "0 24px 80px rgba(0,0,0,0.7)" }}>
            ...
            <div style={{ padding: "20px 24px" }}>
              ...
              <div style={{ fontSize: 10, color: "#c8a84b", letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 12, fontFamily: "'Bebas Neue', sans-serif" }}>Arreglos ({ordenDetalle.arreglos.length})</div>
              ...
              {ordenDetalle.arreglos.length > 0 && (
                <div style={{ textAlign: "right", color: "#888", fontSize: 12, marginTop: 4 }}>
                  Total: <strong style={{ color: "#fd79a8" }}>{ordenDetalle.arreglos.reduce((s, a) => s + (Number(a.horas) || 0), 0).toFixed(1)} hs</strong>
                </div>
              )}
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.88)", display: "flex", alignItems: "flex-start", justifyContent: "center", zIndex: 800, padding: "24px 16px", overflowY: "auto" }}
          ...
          <div style={{ background: "#141414", borderRadius: 16, width: "100%", maxWidth: 600, border: "1px solid #c8a84b33", boxShadow: "0 24px 80px rgba(0,0,0,0.8)" }}>
            <div style={{ padding: "22px 26px 18px", borderBottom: "1px solid #1e1e1e", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
                <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 22, letterSpacing: 1 }}>{editId ? "Editar Orden" : "Nueva Orden"}</div>
                <div style={{ fontSize: 12, color: "#c8a84b", marginTop: 1 }}>{tallerActivo.nombre}</div>
              </div>
